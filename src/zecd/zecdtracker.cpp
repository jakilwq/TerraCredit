...
CzCREDITTracker::CzCREDITTracker(std::string strWalletFile)
{
...
CzCREDITTracker::~CzCREDITTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCREDITTracker::Init()
{
...
bool CzCREDITTracker::Archive(CMintMeta& meta)
{
...
bool CzCREDITTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCREDITTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCREDITTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCREDITTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCREDITTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCREDITTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCREDITTracker::GetSerialHashes()
{
...
CAmount CzCREDITTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCREDITTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCREDITTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCREDITTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCREDITTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCREDITTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCREDITTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCREDITTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCREDITTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCREDITTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCREDITTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCREDITWallet* zCREDITWallet)
{
    bool iszCREDITWalletInitialized = (NULL != zCREDITWallet);
    CMintMeta meta;
    ...
    if (! iszCREDITWalletInitialized)
        zCREDITWallet = new CzCREDITWallet(strWalletFile);
    meta.isSeedCorrect = zCREDITWallet->CheckSeed(dMint);
    if (! iszCREDITWalletInitialized)
        delete zCREDITWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCREDITTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCREDITTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCREDITTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCREDITTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCREDITTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCREDITTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCREDITWallet* zCREDITWallet = new CzCREDITWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCREDITWallet);
        }
        delete zCREDITWallet;
        LogPrint("zero", "%s: added %d dzcredit from DB\n", __func__, listDeterministicDB.size());
    }
