...
CzCREDITWallet::CzCREDITWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcredit seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCREDITSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCREDITSeed_deprecated()) {
                    LogPrintf("%s: Updated zCREDIT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcredit seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcredit wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCREDITWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCREDITCount(nCountLastUsed);
    else if (!walletdb.ReadZCREDITCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCREDITWallet::Lock()
{
    seedMaster = 0;
}

void CzCREDITWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCREDITWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCREDIT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCREDITWallet::LoadMintPoolFromDB()
{
...
void CzCREDITWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCREDITWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCREDITWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcreditTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCREDITWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCREDIT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcreditTracker which also adds to database
    pwalletMain->zcreditTracker->Add(dMint, true);
...
void CzCREDITWallet::SeedToZCREDIT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCREDITWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCREDITWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZCREDITCount(nCountLastUsed);
...
void CzCREDITWallet::GenerateDeterministicZCREDIT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCREDITWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCREDITWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCREDITWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
