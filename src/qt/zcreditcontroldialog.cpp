...
#include "ui_zcreditcontroldialog.h"
...
std::set<std::string> ZCreditControlDialog::setSelectedMints;
std::set<CMintMeta> ZCreditControlDialog::setMints;

bool CZCreditControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZCreditControlDialog::COLUMN_DENOMINATION || column == ZCreditControlDialog::COLUMN_VERSION || column == ZCreditControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZCreditControlDialog::ZCreditControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCreditControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zCREDIT Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZCredit->setProperty("cssClass", "text-main-purple");
    ui->labelZCredit_int->setProperty("cssClass", "text-main-purple");
...
ZCreditControlDialog::~ZCreditControlDialog()
{
    delete ui;
}

void ZCreditControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCreditControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZCreditControlWidgetItem* itemDenom(new CZCreditControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZCreditControlWidgetItem *itemMint = new CZCreditControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcreditTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcreditTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCREDIT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCREDIT seed used to mint this zCREDIT is not the same as currently hold in the wallet";
            else
...
void ZCreditControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCreditControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCredit_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZCreditControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCreditControlDialog::GetSelectedMints()
{
...
void ZCreditControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
