...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::CREDIT);
    ...
    if (settings.contains("nAnonymizeTerraCreditAmount") || reset)
        SoftSetArg("-anonymizeterracreditamount", settings.value("nAnonymizeTerraCreditAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in terracredit.cpp during shotdown to also remove the window positions
