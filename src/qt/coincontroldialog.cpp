                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select CREDIT Denominations to Spend");
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::CREDIT;
    ...
    QString toolTip4 = tr("Can vary +/- %1 ucredit per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show TERRACREDIT address. In tree mode, address is not shown again for direct wallet address outputs
