...
#include "qt/terracredit/send.h"
#include "qt/terracredit/forms/ui_send.h"
#include "qt/terracredit/addnewcontactdialog.h"
#include "qt/terracredit/qtutils.h"
#include "qt/terracredit/sendchangeaddressdialog.h"
#include "qt/terracredit/optionbutton.h"
#include "qt/terracredit/sendconfirmdialog.h"
#include "qt/terracredit/myaddressrow.h"
#include "qt/terracredit/guitransactionsutils.h"
...
#include "zcreditcontroldialog.h"

SendWidget::SendWidget(TERRACREDITGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("CREDIT");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zCREDIT");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (CREDIT) or private coins (zCREDIT)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a TERRACREDIT address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 CREDIT");
    ...
    setCssProperty(coinIcon, "coin-icon-credit");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onCREDITSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onCREDITSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send CREDIT");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zCREDIT");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcredit = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcredit));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcredit ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendCredit = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendCredit ? AskPassphraseDialog::Context::Send_CREDIT : AskPassphraseDialog::Context::Send_zCREDIT, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendCredit) ? send(recipients) : sendZcredit(recipients)) {
...
bool SendWidget::sendZcredit(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zCREDIT is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zCREDIT selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZCreditControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCreditControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcredit(
        ...
        inform(tr("zCREDIT transaction sent!"));
        ZCreditControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZCREDIT_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zCREDIT require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCREDIT transaction
        ...
        emit message("zCREDIT transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isCREDIT){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any CREDIT to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZCreditControlDialog *zCreditControl = new ZCreditControlDialog(this);
            zCreditControl->setModel(walletModel);
            zCreditControl->exec();
            ui->btnCoinControl->setActive(!ZCreditControlDialog::setSelectedMints.empty());
            zCreditControl->deleteLater();
        } else {
            inform(tr("You don't have any zCREDIT in your balance to select."));
        }
    }
}
...
void SendWidget::onCREDITSelected(bool _isCREDIT){
    isCREDIT = _isCREDIT;
    setCssProperty(coinIcon, _isCREDIT ? "coin-icon-credit" : "coin-icon-zcredit");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress creditAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(creditAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(creditAdd.Get(), dialog->getLabel().toStdString(), "send")) {
