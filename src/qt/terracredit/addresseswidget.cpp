...
#include "qt/terracredit/forms/ui_addresseswidget.h"
#include "qt/terracredit/addresslabelrow.h"
#include "qt/terracredit/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(TERRACREDITGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a TERRACREDIT address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress creditAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(creditAdd.Get(), label.toUtf8().constData(),
                creditAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
