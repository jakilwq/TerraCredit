...
#include "qt/terracredit/dashboardwidget.h"
#include "qt/terracredit/forms/ui_dashboardwidget.h"
#include "qt/terracredit/sendconfirmdialog.h"
#include "qt/terracredit/txrow.h"
...
DashboardWidget::DashboardWidget(TERRACREDITGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of CREDIT staked."));  // Amount of CREDIT and zCREDIT staked.
    setCssSubtitleScreen(ui->labelMessage);
    // ui->labelMessage->setVisible(false);
    
    setCssProperty(ui->labelSquareCredit, "square-chart-credit");
    setCssProperty(ui->labelSquarezCredit, "square-chart-zcredit");
    setCssProperty(ui->labelCredit, "text-chart-credit");
    setCssProperty(ui->labelZcredit, "text-chart-zcredit");

    // ui->labelCredit->setVisible(false);
    // ui->labelZcredit->setVisible(false);
    ...
    ui->labelAmountZcredit->setText("0 zCREDIT");
    ui->labelAmountCredit->setText("0 CREDIT");
    setCssProperty(ui->labelAmountCredit, "text-stake-credit-disable");
    setCssProperty(ui->labelAmountZcredit, "text-stake-zcredit-disable");
    ...
    ui->btnHowTo->setText(tr("How to get CREDIT or zCREDIT"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("CREDIT")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isCredit = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZCREDIT;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair CREDIT, zCREDIT
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            credit = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcredit = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalCredit += pair.first;
            chartData->totalZcredit += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesCredit.append(credit);
        chartData->valueszCredit.append(zcredit);

        int max = std::max(credit, zcredit);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("CREDIT");
    set1 = new QBarSet("zCREDIT");
    ...
    set0->append(chartData->valuesCredit);
    set1->append(chartData->valueszCredit);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalCredit > 0 || chartData->totalZcredit > 0) {
        setCssProperty(ui->labelAmountCredit, "text-stake-credit");
        setCssProperty(ui->labelAmountZcredit, "text-stake-zcredit");
    } else {
        setCssProperty(ui->labelAmountCredit, "text-stake-credit-disable");
        setCssProperty(ui->labelAmountZcredit, "text-stake-zcredit-disable");
    }
    forceUpdateStyle({ui->labelAmountCredit, ui->labelAmountZcredit});
    ui->labelAmountCredit->setText(GUIUtil::formatBalance(chartData->totalCredit, nDisplayUnit));
    ui->labelAmountZcredit->setText(GUIUtil::formatBalance(chartData->totalZcredit, nDisplayUnit, true));
