...
#include "qt/terracredit/privacywidget.h"
#include "qt/terracredit/forms/ui_privacywidget.h"
#include "qt/terracredit/qtutils.h"
#include "guiutil.h"
#include "qt/terracredit/denomgenerationdialog.h"
#include "qt/terracredit/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(TERRACREDITGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zCREDIT anonymizes your CREDIT by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zCREDIT or convert back to CREDIT"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 CREDIT ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zCREDIT");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zCREDIT");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zCREDIT");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zCREDIT");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zCREDIT");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zCREDIT");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zCREDIT");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zCREDIT");
    ...
    ui->btnTotalzCREDIT->setSubTitleClassAndText("text-subtitle", "Show denominations of zCREDIT owned.");
    ui->btnTotalzCREDIT->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select CREDIT outputs to mint into zCREDIT.");
    ...
    connect(ui->btnTotalzCREDIT, SIGNAL(clicked()), this, SLOT(onTotalZcreditClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zCREDIT");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of CREDIT to mint into zCREDIT"));
    }else{
        btnText = tr("Convert back to CREDIT");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zCREDIT to convert back into CREDIT"));
    }
...
void PrivacyWidget::onTotalZcreditClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzCREDIT->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzCREDIT->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zCREDIT is currently undergoing maintenance"));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zCREDIT minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcredit(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zCREDIT converted back to CREDIT"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCREDIT </b>";
    ...
    ui->btnTotalzCREDIT->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
