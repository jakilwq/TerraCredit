...
#include "qt/terracredit/topbar.h"
#include "askpassphrasedialog.h"
#include "qt/terracredit/forms/ui_topbar.h"
#include "qt/terracredit/lockunlock.h"
#include "qt/terracredit/qtutils.h"
#include "qt/terracredit/receivedialog.h"
#include <QPixmap>
...
TopBar::TopBar(TERRACREDITGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                             ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopCredit, ui->labelAmountTopzCredit}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("TERRACREDIT")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    CAmount creditAvailableBalance = balance + delegatedBalance - nLockedBalance;

    // zCREDIT Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalCredit = GUIUtil::formatBalance(creditAvailableBalance, nDisplayUnit);
    QString totalzCredit = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopCredit->setText(totalCredit);
    ui->labelAmountTopzCredit->setText(totalzCredit);

    // Expanded
    ui->labelAmountCredit->setText(totalCredit);
    ui->labelAmountzCredit->setText(totalzCredit);

    ui->labelPendingCredit->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzCredit->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureCredit->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezCredit->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
