...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "terracredit" is a composite category enabling all TERRACREDIT-related debug output
            if (ptrCategory->count(std::string("terracredit"))) {
...
#else
    const char* pszModule = "terracredit";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\TERRACREDIT
// Windows >= Vista: C:\Users\Username\AppData\Roaming\TERRACREDIT
// Mac: ~/Library/Application Support/TERRACREDIT
// Unix: ~/.terracredit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TERRACREDIT";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "terracredit.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            ...
            strHeader += "\naddnode=eva.terra-credit.com\naddnode=luke.terra-credit.com\naddnode=seed01.terra-credit.com\naddnode=seed02.terra-credit.com\naddnode=seed03.terra-credit.com\naddnode=seed04.terra-credit.com\naddnode=seed05.terra-credit.com\naddnode=seed06.terra-credit.com\n";
            strHeader += "txindex=1\n";
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override terracredit.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "terracreditd.pid"));
