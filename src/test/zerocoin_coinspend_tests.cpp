...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzCREDITWallet *czCREDITWallet = new CzCREDITWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czCREDITWallet->GenerateDeterministicZCREDIT(denom, coin, dMint, true);
        czCREDITWallet->UpdateCount();
