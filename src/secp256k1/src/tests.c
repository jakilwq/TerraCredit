...
#include "openssl/creditsa.h"
#include "openssl/obj_mac.h"
# if OPENSSL_VERSION_NUMBER < 0x10100000L
void CREDITSA_SIG_get0(const CREDITSA_SIG *sig, const BIGNUM **pr, const BIGNUM **ps) {*pr = sig->r; *ps = sig->s;}
# endif
#endif
...
void run_context_tests(void) {
    ...
    secp256k1_creditsa_signature sig;
    ...
    CHECK(secp256k1_creditsa_sign(vrfy, &sig, ctmp, ctmp, NULL, NULL) == 0);
    CHECK(ecount == 2);
    VG_UNDEF(&sig, sizeof(sig));
    CHECK(secp256k1_creditsa_sign(sign, &sig, ctmp, ctmp, NULL, NULL) == 1);
    VG_CHECK(&sig, sizeof(sig));
    CHECK(ecount2 == 10);
    CHECK(secp256k1_creditsa_verify(sign, &sig, ctmp, &pubkey) == 0);
    CHECK(ecount2 == 11);
    CHECK(secp256k1_creditsa_verify(vrfy, &sig, ctmp, &pubkey) == 1);
    CHECK(ecount == 2);
    ...
    } while(!secp256k1_creditsa_sig_sign(&both->ecmult_gen_ctx, &sigr, &sigs, &key, &msg, &nonce, NULL));

    /* try signing */
    CHECK(secp256k1_creditsa_sig_sign(&sign->ecmult_gen_ctx, &sigr, &sigs, &key, &msg, &nonce, NULL));
    CHECK(secp256k1_creditsa_sig_sign(&both->ecmult_gen_ctx, &sigr, &sigs, &key, &msg, &nonce, NULL));

    /* try verifying */
    CHECK(secp256k1_creditsa_sig_verify(&vrfy->ecmult_ctx, &sigr, &sigs, &pub, &msg));
    CHECK(secp256k1_creditsa_sig_verify(&both->ecmult_ctx, &sigr, &sigs, &pub, &msg));
...
void run_sha256_tests(void) {
    static const char *inputs[8] = {
        ...
        "abcdbcdcreditefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq",
...
void random_sign(secp256k1_scalar *sigr, secp256k1_scalar *sigs, const secp256k1_scalar *key, const secp256k1_scalar *msg, int *recid) {
    ...
    do {
        random_scalar_order_test(&nonce);
    } while(!secp256k1_creditsa_sig_sign(&ctx->ecmult_gen_ctx, sigr, sigs, key, msg, &nonce, recid));
}

void test_creditsa_sign_verify(void) {
    secp256k1_gej pubj;
    secp256k1_ge pub;
    secp256k1_scalar one;
    ...
    CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &pub, &msg));
    secp256k1_scalar_set_int(&one, 1);
    secp256k1_scalar_add(&msg, &msg, &one);
    CHECK(!secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &pub, &msg));
}

void run_creditsa_sign_verify(void) {
    int i;
    for (i = 0; i < 10*count; i++) {
        test_creditsa_sign_verify();
    }
}
...
int is_empty_signature(const secp256k1_creditsa_signature *sig) {
    static const unsigned char res[sizeof(secp256k1_creditsa_signature)] = {0};
    return memcmp(sig, res, sizeof(secp256k1_creditsa_signature)) == 0;
}

void test_creditsa_end_to_end(void) {
    unsigned char extra[32] = {0x00};
    unsigned char privkey[32];
    unsigned char message[32];
    unsigned char privkey2[32];
    secp256k1_creditsa_signature signature[6];
    ...
    /* Sign. */
    CHECK(secp256k1_creditsa_sign(ctx, &signature[0], message, privkey, NULL, NULL) == 1);
    CHECK(secp256k1_creditsa_sign(ctx, &signature[4], message, privkey, NULL, NULL) == 1);
    CHECK(secp256k1_creditsa_sign(ctx, &signature[1], message, privkey, NULL, extra) == 1);
    extra[31] = 1;
    CHECK(secp256k1_creditsa_sign(ctx, &signature[2], message, privkey, NULL, extra) == 1);
    extra[31] = 0;
    extra[0] = 1;
    CHECK(secp256k1_creditsa_sign(ctx, &signature[3], message, privkey, NULL, extra) == 1);
    ...
    /* Verify. */
    CHECK(secp256k1_creditsa_verify(ctx, &signature[0], message, &pubkey) == 1);
    CHECK(secp256k1_creditsa_verify(ctx, &signature[1], message, &pubkey) == 1);
    CHECK(secp256k1_creditsa_verify(ctx, &signature[2], message, &pubkey) == 1);
    CHECK(secp256k1_creditsa_verify(ctx, &signature[3], message, &pubkey) == 1);
    /* Test lower-S form, malleate, verify and fail, test again, malleate again */
    CHECK(!secp256k1_creditsa_signature_normalize(ctx, NULL, &signature[0]));
    secp256k1_creditsa_signature_load(ctx, &r, &s, &signature[0]);
    secp256k1_scalar_negate(&s, &s);
    secp256k1_creditsa_signature_save(&signature[5], &r, &s);
    CHECK(secp256k1_creditsa_verify(ctx, &signature[5], message, &pubkey) == 0);
    CHECK(secp256k1_creditsa_signature_normalize(ctx, NULL, &signature[5]));
    CHECK(secp256k1_creditsa_signature_normalize(ctx, &signature[5], &signature[5]));
    CHECK(!secp256k1_creditsa_signature_normalize(ctx, NULL, &signature[5]));
    CHECK(!secp256k1_creditsa_signature_normalize(ctx, &signature[5], &signature[5]));
    CHECK(secp256k1_creditsa_verify(ctx, &signature[5], message, &pubkey) == 1);
    secp256k1_scalar_negate(&s, &s);
    secp256k1_creditsa_signature_save(&signature[5], &r, &s);
    CHECK(!secp256k1_creditsa_signature_normalize(ctx, NULL, &signature[5]));
    CHECK(secp256k1_creditsa_verify(ctx, &signature[5], message, &pubkey) == 1);
    ...
    CHECK(secp256k1_creditsa_signature_serialize_der(ctx, sig, &siglen, &signature[0]) == 1);
    memset(&signature[0], 0, sizeof(signature[0]));
    CHECK(secp256k1_creditsa_signature_parse_der(ctx, &signature[0], sig, siglen) == 1);
    CHECK(secp256k1_creditsa_verify(ctx, &signature[0], message, &pubkey) == 1);
    /* Serialize/destroy/parse DER and verify again. */
    siglen = 74;
    CHECK(secp256k1_creditsa_signature_serialize_der(ctx, sig, &siglen, &signature[0]) == 1);
    sig[secp256k1_rand_int(siglen)] += 1 + secp256k1_rand_int(255);
    CHECK(secp256k1_creditsa_signature_parse_der(ctx, &signature[0], sig, siglen) == 0 ||
          secp256k1_creditsa_verify(ctx, &signature[0], message, &pubkey) == 0);
}
...
void run_creditsa_end_to_end(void) {
    int i;
    for (i = 0; i < 64*count; i++) {
        test_creditsa_end_to_end();
    }
}

int test_creditsa_der_parse(const unsigned char *sig, size_t siglen, int certainly_der, int certainly_not_der) {
    ...
    parsed_der = secp256k1_creditsa_signature_parse_der(ctx, &sig_der, sig, siglen);
    if (parsed_der) {
        ret |= (!secp256k1_creditsa_signature_serialize_compact(ctx, compact_der, &sig_der)) << 0;
        valid_der = (memcmp(compact_der, zeroes, 32) != 0) && (memcmp(compact_der + 32, zeroes, 32) != 0);
    }
    if (valid_der) {
        ret |= (!secp256k1_creditsa_signature_serialize_der(ctx, roundtrip_der, &len_der, &sig_der)) << 1;
        roundtrips_der = (len_der == siglen) && memcmp(roundtrip_der, sig, siglen) == 0;
    }

    parsed_der_lax = creditsa_signature_parse_der_lax(ctx, &sig_der_lax, sig, siglen);
    if (parsed_der_lax) {
        ret |= (!secp256k1_creditsa_signature_serialize_compact(ctx, compact_der_lax, &sig_der_lax)) << 10;
        valid_der_lax = (memcmp(compact_der_lax, zeroes, 32) != 0) && (memcmp(compact_der_lax + 32, zeroes, 32) != 0);
    }
    if (valid_der_lax) {
        ret |= (!secp256k1_creditsa_signature_serialize_der(ctx, roundtrip_der_lax, &len_der_lax, &sig_der_lax)) << 11;
...
#ifdef ENABLE_OPENSSL_TESTS
    sig_openssl = CREDITSA_SIG_new();
    sigptr = sig;
    parsed_openssl = (d2i_CREDITSA_SIG(&sig_openssl, &sigptr, siglen) != NULL);
    if (parsed_openssl) {
        CREDITSA_SIG_get0(sig_openssl, &r, &s);
...
void run_creditsa_der_parse(void) {
    int i,j;
    for (i = 0; i < 200 * count; i++) {
        ...
        for (j = 0; j < 16; j++) {
            ...
            ret = test_creditsa_der_parse(buffer, buflen, certainly_der, certainly_not_der);
            if (ret != 0) {
                size_t k;
...
void test_creditsa_edge_cases(void) {
    int t;
    secp256k1_creditsa_signature sig;

    /* Test the case where CREDITSA recomputes a point that is infinity. */
    {
        secp256k1_gej keyj;
        secp256k1_ge key;
        ...
        msg = ss;
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key, &msg) == 0);
    }
    ...
    /* Verify signature with message 0 passes. */
    {
        ...
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key, &msg) == 1);
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key2, &msg) == 1);
        secp256k1_scalar_negate(&ss, &ss);
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key, &msg) == 1);
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key2, &msg) == 1);
        secp256k1_scalar_set_int(&ss, 1);
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key, &msg) == 0);
        CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sr, &ss, &key2, &msg) == 0);
    }
    ...
    /* Signature where s would be zero. */
    {
        ...
        ecount = 0;
        secp256k1_context_set_illegal_callback(ctx, counting_illegal_callback_fn, &ecount);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, precomputed_nonce_function, nonce) == 0);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, precomputed_nonce_function, nonce2) == 0);
        msg[31] = 0xaa;
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, precomputed_nonce_function, nonce) == 1);
        CHECK(ecount == 0);
        CHECK(secp256k1_creditsa_sign(ctx, NULL, msg, key, precomputed_nonce_function, nonce2) == 0);
        CHECK(ecount == 1);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, NULL, key, precomputed_nonce_function, nonce2) == 0);
        CHECK(ecount == 2);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, NULL, precomputed_nonce_function, nonce2) == 0);
        CHECK(ecount == 3);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, precomputed_nonce_function, nonce2) == 1);
        CHECK(secp256k1_ec_pubkey_create(ctx, &pubkey, key) == 1);
        CHECK(secp256k1_creditsa_verify(ctx, NULL, msg, &pubkey) == 0);
        CHECK(ecount == 4);
        CHECK(secp256k1_creditsa_verify(ctx, &sig, NULL, &pubkey) == 0);
        CHECK(ecount == 5);
        CHECK(secp256k1_creditsa_verify(ctx, &sig, msg, NULL) == 0);
        CHECK(ecount == 6);
        CHECK(secp256k1_creditsa_verify(ctx, &sig, msg, &pubkey) == 1);
        CHECK(ecount == 6);
        CHECK(secp256k1_ec_pubkey_create(ctx, &pubkey, NULL) == 0);
        CHECK(ecount == 7);
        /* That pubkeyload fails via an ARGCHECK is a little odd but makes sense because pubkeys are an opaque data type. */
        CHECK(secp256k1_creditsa_verify(ctx, &sig, msg, &pubkey) == 0);
        CHECK(ecount == 8);
        siglen = 72;
        CHECK(secp256k1_creditsa_signature_serialize_der(ctx, NULL, &siglen, &sig) == 0);
        CHECK(ecount == 9);
        CHECK(secp256k1_creditsa_signature_serialize_der(ctx, signature, NULL, &sig) == 0);
        CHECK(ecount == 10);
        CHECK(secp256k1_creditsa_signature_serialize_der(ctx, signature, &siglen, NULL) == 0);
        CHECK(ecount == 11);
        CHECK(secp256k1_creditsa_signature_serialize_der(ctx, signature, &siglen, &sig) == 1);
        CHECK(ecount == 11);
        CHECK(secp256k1_creditsa_signature_parse_der(ctx, NULL, signature, siglen) == 0);
        CHECK(ecount == 12);
        CHECK(secp256k1_creditsa_signature_parse_der(ctx, &sig, NULL, siglen) == 0);
        CHECK(ecount == 13);
        CHECK(secp256k1_creditsa_signature_parse_der(ctx, &sig, signature, siglen) == 1);
        CHECK(ecount == 13);
        siglen = 10;
        /* Too little room for a signature does not fail via ARGCHECK. */
        CHECK(secp256k1_creditsa_signature_serialize_der(ctx, signature, &siglen, &sig) == 0);
        CHECK(ecount == 13);
        ecount = 0;
        CHECK(secp256k1_creditsa_signature_normalize(ctx, NULL, NULL) == 0);
        CHECK(ecount == 1);
        CHECK(secp256k1_creditsa_signature_serialize_compact(ctx, NULL, &sig) == 0);
        CHECK(ecount == 2);
        CHECK(secp256k1_creditsa_signature_serialize_compact(ctx, signature, NULL) == 0);
        CHECK(ecount == 3);
        CHECK(secp256k1_creditsa_signature_serialize_compact(ctx, signature, &sig) == 1);
        CHECK(ecount == 3);
        CHECK(secp256k1_creditsa_signature_parse_compact(ctx, NULL, signature) == 0);
        CHECK(ecount == 4);
        CHECK(secp256k1_creditsa_signature_parse_compact(ctx, &sig, NULL) == 0);
        CHECK(ecount == 5);
        CHECK(secp256k1_creditsa_signature_parse_compact(ctx, &sig, signature) == 1);
        CHECK(ecount == 5);
        memset(signature, 255, 64);
        CHECK(secp256k1_creditsa_signature_parse_compact(ctx, &sig, signature) == 0);
        CHECK(ecount == 5);
        secp256k1_context_set_illegal_callback(ctx, NULL, NULL);
    }
    ...
    for (t = 0; t < 2; t++) {
        ...
        unsigned char msg[32];
        secp256k1_creditsa_signature sig2;
        ...
        memset(key, 0xFF, 32);
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, NULL, extra) == 0);
        ...
        key[31] = 1;
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, nonce_function_test_fail, extra) == 0);
        CHECK(is_empty_signature(&sig));
        /* The retry loop successfully makes its way to the first good value. */
        CHECK(secp256k1_creditsa_sign(ctx, &sig, msg, key, nonce_function_test_retry, extra) == 1);
        CHECK(!is_empty_signature(&sig));
        CHECK(secp256k1_creditsa_sign(ctx, &sig2, msg, key, nonce_function_rfc6979, extra) == 1);
        CHECK(!is_empty_signature(&sig2));
        CHECK(memcmp(&sig, &sig2, sizeof(sig)) == 0);
        /* The default nonce function is deterministic. */
        CHECK(secp256k1_creditsa_sign(ctx, &sig2, msg, key, NULL, extra) == 1);
        CHECK(!is_empty_signature(&sig2));
        CHECK(memcmp(&sig, &sig2, sizeof(sig)) == 0);
        ...
        for(i = 0; i < 256; i++) {
            ...
            CHECK(secp256k1_creditsa_sign(ctx, &sig2, msg, key, NULL, extra) == 1);
            CHECK(!is_empty_signature(&sig2));
            secp256k1_creditsa_signature_load(ctx, &sr[i], &ss, &sig2);
            for (j = 0; j < i; j++) {
...
void run_creditsa_edge_cases(void) {
    test_creditsa_edge_cases();
}
...
void test_creditsa_openssl(void) {
    secp256k1_gej qj;
    secp256k1_ge q;
    secp256k1_scalar sigr, sigs;
    secp256k1_scalar one;
    ...
    CHECK(ec_key != NULL);
    CHECK(CREDITSA_sign(0, message, sizeof(message), signature, &sigsize, ec_key));
    CHECK(secp256k1_creditsa_sig_parse(&sigr, &sigs, signature, sigsize));
    CHECK(secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &q, &msg));
    secp256k1_scalar_set_int(&one, 1);
    secp256k1_scalar_add(&msg2, &msg, &one);
    CHECK(!secp256k1_creditsa_sig_verify(&ctx->ecmult_ctx, &sigr, &sigs, &q, &msg2));

    random_sign(&sigr, &sigs, &key, &msg, NULL);
    CHECK(secp256k1_creditsa_sig_serialize(signature, &secp_sigsize, &sigr, &sigs));
    CHECK(CREDITSA_verify(0, message, sizeof(message), signature, secp_sigsize, ec_key) == 1);

    EC_KEY_free(ec_key);
}

void run_creditsa_openssl(void) {
    int i;
    for (i = 0; i < 10*count; i++) {
        test_creditsa_openssl();
    }
}
#endif

#ifdef ENABLE_MODULE_CREDITH
# include "modules/credith/tests_impl.h"
#endif
...
int main(int argc, char **argv) {
    ...
    /* creditsa tests */
    run_random_pubkeys();
    run_creditsa_der_parse();
    run_creditsa_sign_verify();
    run_creditsa_end_to_end();
    run_creditsa_edge_cases();
#ifdef ENABLE_OPENSSL_TESTS
    run_creditsa_openssl();
#endif

#ifdef ENABLE_MODULE_RECOVERY
    /* CREDITSA pubkey recovery tests */
    run_recovery_tests();
#endif
