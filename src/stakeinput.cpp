...
CZCreditStake::CZCreditStake(const libzerocoin::CoinSpend& spend)
{
...
int CZCreditStake::GetChecksumHeightFromMint()
{
...
int CZCreditStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZCreditStake::GetChecksum()
{
    return nChecksum;
}

// The zCREDIT block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZCreditStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZCreditStake::GetValue()
{
    return denom * COIN;
}
...
bool CZCreditStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zCREDIT staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZCreditStake::GetUniqueness()
{
    //The unique identifier for a zCREDIT is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZCreditStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZCreditStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zCREDIT that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCREDITOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCREDIT output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCREDIT", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCREDITOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCREDIT output", __func__);
...
bool CZCreditStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZCreditStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzCREDITTracker* zcreditTracker = pwallet->zcreditTracker.get();
    CMintMeta meta;
    if (!zcreditTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcreditTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!CREDIT Stake
bool CCreditStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CCreditStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CCreditStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CCreditStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CCreditStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CCreditStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CCreditStake::GetUniqueness()
{
    //The unique identifier for a CREDIT stake is the outpoint
...
CBlockIndex* CCreditStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
