...
/**
 * TERRACREDIT RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZcreditSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
// extern UniValue setzcreditseed(const UniValue& params, bool fHelp);
// extern UniValue getzcreditseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
// extern UniValue searchdzcredit(const UniValue& params, bool fHelp);
// extern UniValue dzcreditstate(const UniValue& params, bool fHelp);
