...
#include "zcredit/zcreditmodule.h"
...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zcreditspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 100000 CREDIT at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCREDIT send
enum ZerocoinSpendStatus {
    ZCREDIT_SPEND_OKAY = 0,                            // No error
    ZCREDIT_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCREDIT_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCREDIT_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCREDIT_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCREDIT_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCREDIT_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCREDIT_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCREDIT_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCREDIT_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCREDIT_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCREDIT_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCREDIT_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCREDIT_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCREDIT_SPENT_USED_ZCREDIT = 14,                      // Coin has already been spend
    ZCREDIT_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCREDIT_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCreditBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCREDITOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzcreditAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzCREDITTracker> zcreditTracker;
    ...
    void setZWallet(CzCREDITWallet* zwallet);
    CzCREDITWallet* getZWallet();
    bool isZeromintEnabled();
    void setZCreditAutoBackups(bool fEnabled);
    ...
    /// Get 100000 CREDIT output and keys which can be used for the Masternode
    ...
    /** zCREDIT reset */
    boost::signals2::signal<void()> NotifyzCREDITReset;
