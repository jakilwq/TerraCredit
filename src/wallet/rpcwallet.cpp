...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new TERRACREDIT address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"terracreditaddress\"    (string) The new terracredit address\n"
...
UniValue getnewstakingaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new TERRACREDIT cold staking address for receiving delegated cold stakes.\n"
...
UniValue delegatoradd(const UniValue& params, bool fHelp)
{
    ...
    if (!address.IsValid() || address.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TERRACREDIT address");

    CKeyID keyID;
    if (!address.GetKeyID(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from TERRACREDIT address");
...
UniValue delegatorremove(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->HasAddressBook(keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get TERRACREDIT address from addressBook");
...
UniValue listdelegators(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) TERRACREDIT address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current TERRACREDIT address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"terracreditaddress\"   (string) The account terracredit address\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new TERRACREDIT address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"terracreditaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"terracreditaddress\"  (string, required) The terracredit address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"terracreditaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"terracreditaddress\"  (string, required) The terracredit address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"terracreditaddress\"  (string) a terracredit address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"terracreditaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"terracreditaddress\"  (string, required) The terracredit address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in CREDIT to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CWalletTx& wtxNew, CReserveKey& reservekey)
{
    ...
    if (!stakeAddr.IsValid() || !stakeAddr.IsStakingAddress())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TERRACREDIT staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!ownerAddr.IsValid() || ownerAddr.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TERRACREDIT spending address");
...
UniValue delegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"stakingaddress\"      (string, required) The terracredit staking address to delegate.\n"
            "2. \"amount\"              (numeric, required) The amount in CREDIT to delegate for staking. eg 100\n"
            "3. \"owneraddress\"        (string, optional) The terracredit address corresponding to the key that will be able to spend the stake. \n"
...
UniValue rawdelegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "           \"terracreditaddress\"        (string) terracredit address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"terracreditaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"terracreditaddress\",     (string) The terracredit address\n"
            "      amount,                 (numeric) The amount in CREDIT\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"terracreditaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"terracreditaddress\"  (string, required) The terracredit address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"terracreditaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given terracreditaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"terracreditaddress\"  (string, required) The terracredit address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in CREDIT received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in CREDIT received for this account.\n"
...
UniValue getcoldstakingbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in CREDIT received for this account in P2CS contracts.\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of CREDIT to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 CREDIT from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 CREDIT from timotei to akiko with a comment\n" +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toterracreditaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nSent an amount from an account to a terracredit address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toterracreditaddress\"  (string, required) The terracredit address to send funds to.\n"
            "3. amount                (numeric, required) The amount in CREDIT. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 CREDIT from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The terracredit address is the key, the numeric amount in CREDIT is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!address.IsValid() || address.IsStakingAddress())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid TERRACREDIT address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a TERRACREDIT address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of terracredit addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) terracredit address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"terracreditaddress\"  (string) A terracredit address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in CREDIT received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"terracreditaddress\",    (string) The terracredit address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in CREDIT. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in CREDIT. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"terracreditaddress\",    (string) The terracredit address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in CREDIT. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in CREDIT. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in CREDIT\n"
            ...
            "      \"address\" : \"terracreditaddress\",   (string) The terracredit address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in CREDIT\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            HelpExampleCli("signmessage", "\"terracreditaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; terracredit server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in CREDIT/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total CREDIT balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the CREDIT balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the CREDIT balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in CREDIT\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in CREDIT\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in CREDIT\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in CREDIT\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,                      (numeric) the transaction fee configuration, set in CREDIT/kB\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same TERRACREDIT address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <TERRACREDIT Address> <percent>\n"
...
//         throw std::runtime_error(
//             "getzerocoinbalance\n"
//             "\nReturn the wallet's total zCREDIT balance.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nResult:\n"
//             "amount         (numeric) Total zCREDIT balance.\n"
...
//             "\nList all zCREDIT mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    \"zCREDIT ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zcreditTracker->ListMints(true, fMatureOnly, true);
...
//             objMint.push_back(Pair("zCREDIT ID", m.hashPubcoin.GetHex()));     // PubCoin
...
//                     pwalletMain->zcreditTracker->UpdateState(m);
//                 }
//             }
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zcreditTracker->ListMints(true, true, true);
...
//         throw std::runtime_error(
//             "listspentzerocoins\n"
//             "\nList all the spent zCREDIT mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "\nMint the specified zCREDIT amount\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. amount      (numeric, required) Enter an amount of Credit to convert to zCREDIT\n"
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zCREDIT minting is DISABLED");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zCREDIT is currently disabled due to maintenance.");
...
//             "\nSpend zCREDIT to a CREDIT address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    {\n"
//             "      \"value\": amount,         (numeric) Value in CREDIT.\n"
//             "      \"address\": \"xxx\"         (string) CREDIT address or \"zerocoinmint\" for reminted change.\n"
//             "    }\n"
//             "    ,...\n"
//             "  ]\n"
//             "}\n"
...
//     bool fMintChange = params[1].get_bool();        // Mint change to zCREDIT
//     if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
//         throw JSONRPCError(RPC_WALLET_ERROR, "zCREDIT minting is DISABLED, cannot mint change");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zCREDIT old spend only available in regtest for tests purposes");
//     }

//     return DoZcreditSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
// }
...
//             "\nSpend zCREDIT mints to a CREDIT address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//     return DoZcreditSpend(nAmount, false, true, vMintsSelected, address_str);
// }


// extern UniValue DoZcreditSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
// {
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzCREDITTracker* zcreditTracker = pwalletMain->zcreditTracker.get();
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzCREDITTracker* zcreditTracker = pwalletMain->zcreditTracker.get();
//     std::set<CMintMeta> setMints = zcreditTracker->ListMints(false, false, false);
...
//             "2. \"denomination\"         (integer, optional) Export a specific denomination of zCREDIT\n"

//             "\nResult:\n"
//             "[                   (array of json object)\n"
//             "  {\n"
//             "    \"id\": \"serial hash\",  (string) the mint's zCREDIT serial hash \n"
...
//             "    \"v\": version,   (numeric) The version of the zCREDIT\n"
//             "    \"k\": \"privkey\"  (string) The zCREDIT private key (V2+ zCREDIT only)\n"
//             "  }\n"
//             "  ,...\n"
//             "]\n"
...
//     CzCREDITTracker* zcreditTracker = pwalletMain->zcreditTracker.get();
//     std::set<CMintMeta> setMints = zcreditTracker->ListMints(!fIncludeSpent, false, false);
...
//             "  \"value\": amount    (numeric) The total zCREDIT value of zerocoin mints that were added\n"
//             "}\n"
...
//         pwalletMain->zcreditTracker->Add(mint, true);
//         count++;
...
//         throw std::runtime_error(
//             "reconsiderzerocoins\n"
//             "\nCheck archived zCREDIT list to see if any mints were added to the blockchain.\n" +
//             HelpRequiringPassphrase() + "\n"
...
// UniValue setzcreditseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 1)
//         throw std::runtime_error(
//             "setzcreditseed \"seed\"\n"
//             "\nSet the wallet's deterministic zcredit seed to a specific value.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. \"seed\"        (string, required) The deterministic zcredit seed.\n"

//             "\nResult\n"
//             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

//             "\nExamples\n" +
//             HelpExampleCli("setzcreditseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
//             HelpExampleRpc("setzcreditseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
//     CzCREDITWallet* zwallet = pwalletMain->getZWallet();
...
// UniValue getzcreditseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || !params.empty())
//         throw std::runtime_error(
//             "getzcreditseed\n"
...
//             "\nResult\n"
//             "\"seed\" : s,  (string) The deterministic zCREDIT seed.\n"

//             "\nExamples\n" +
//             HelpExampleCli("getzcreditseed", "") + HelpExampleRpc("getzcreditseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zCREDIT seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zCREDIT to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zCREDIT to generate.\n"
    ...
    CzCREDITWallet* zwallet = pwalletMain->zwalletMain;
...
// UniValue dzcreditstate(const UniValue& params, bool fHelp) {
//     if (fHelp || params.size() != 0)
//         throw std::runtime_error(
//                 "dzcreditstate\n"
//                         "\nThe current state of the mintpool of the deterministic zCREDIT wallet.\n" +
//                 HelpRequiringPassphrase() + "\n"
...
//     obj.push_back(Pair("dzcredit_count", nCount));
...
void static SearchThread(CzCREDITWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZCREDIT(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
// UniValue searchdzcredit(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 3)
//         throw std::runtime_error(
//             "searchdzcredit\n"
//             "\nMake an extended search for deterministically generated zCREDIT that have not yet been recognized by the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments\n"
//             "1. \"count\"       (numeric) Which sequential zCREDIT to start with.\n"
//             "2. \"range\"       (numeric) How many zCREDIT to generate.\n"
//             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

//             "\nExamples\n" +
//             HelpExampleCli("searchdzcredit", "1, 100, 2") + HelpExampleRpc("searchdzcredit", "1, 100, 2"));
...
//     boost::thread_group* dzcreditThreads = new boost::thread_group();
...
//         nPrevThreadEnd = nEnd;
//         dzcreditThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
//     }

//     dzcreditThreads->join_all();

//     zwallet->RemoveMintsFromPool(pwalletMain->zcreditTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) TERRACREDIT address for auto minting\n" +
...
//             "5. \"address\"          (string, optional) TERRACREDIT address to spend to. If not specified, spend to change add.\n"
...
//         throw std::runtime_error(
//             "clearspendcache\n"
//             "\nClear the pre-computed zCREDIT spend cache, and database.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             if (fLocked) {
//                 if (zcreditTracker->ClearSpendCache()) {
