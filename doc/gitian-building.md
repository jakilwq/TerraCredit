...
*Setup instructions for a gitian build of TERRACREDIT Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the TERRACREDIT
...
to the TERRACREDIT GitHub release page.
...
- [Building TERRACREDIT Core](#building-terracredit-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/terracredit-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/terracredit-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/terracredit-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/terracredit-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/terracredit-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/terracredit-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/terracredit-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@terra-credit.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/terracredit-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/terracredit-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/terracredit-osx-*-build.assert
```
